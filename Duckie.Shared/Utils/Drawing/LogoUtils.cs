...
namespace Duckie.Shared.Utils.Drawing;
...
internal class LogoUtils
{
    ...
    public static Bitmap DrawLogo(int edge, int angle = 0, Color? primaryColor = null)
    {
        ...
        using (var g = Graphics.FromImage(bitmap))
        {
            ...
            foreach (var n in Enumerable.Range(0, 6))
            {
                ...
                if (AppEnv.IsDebug)
                {
                    g.DrawRectangles(new Pen(redBrush, 1), new[] { rect });
                }
            }
    ...
    public static Bitmap DrawBadge(Bitmap bitmap, Color color, double sizePercent = 0.5)
    {
        using (var g = Graphics.FromImage(bitmap))
        {
            ...
            var rect = new Rectangle(
                (int)(bitmap.Width * (1 - sizePercent)),
                (int)(bitmap.Height * (1 - sizePercent)),
                (int)(bitmap.Width * sizePercent),
                (int)(bitmap.Height * sizePercent));
            ...
            innerPath.AddEllipse(rect);
            region.Exclude(innerPath);

            // 绘制透明路径
            var eraserBrush = new SolidBrush(Color.FromArgb(0, 0xFF, 0xFF, 0xFF));
            g.CompositingMode = CompositingMode.SourceCopy;
            g.FillRegion(eraserBrush, region);
        }
